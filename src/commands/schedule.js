...
const description = new Discord.MessageEmbed()
    ...
    .addFields({ name: "Requires admin rights: ", value: requireAdminRights, inline: true }, { name: "Usage", value: usageText }, { name: "Parameters", value: parameters }, { name: "Examples: ", value: examples, });

//Command usage
const usage = new Discord.MessageEmbed()
    .setColor('#fcfa65')
    .setTitle(`${Config.command_prefix}${command}`)
    .addFields({ name: "Usage", value: usageText }, { name: "Parameters", value: parameters }, { name: "Examples", value: examples });
...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const notification = new Discord.MessageEmbed()
            ...
            .addFields({ name: 'Session Start', value: `${session.startDateTime.format('DD/MMM/YYYY [-] HH:mm')}` }, { name: 'Session End', value: `${session.endDateTime.format('DD/MMM/YYYY [-] HH:mm' )}` }, { name: 'Session Duration', value: `${session.duration.hours()} Hours ${session.duration.minutes()} Minutes ${session.duration.seconds()} Seconds` });
        ...
        setTimeout(
            function() {
                ...
                const sessionTimeout = new Discord.MessageEmbed()
                    ...
                    .addFields({ name: "Present", value: attendanceList.present }, { name: "On Leave", value: attendanceList.leave }, { name: "Absent", value: attendanceList.absent });
