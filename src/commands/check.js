...
const description = new Discord.MessageEmbed()
    ...
    .addFields({ name: "Requires admin rights: ", value: requireAdminRights, inline: true }, { name: "Usage", value: `${Config.command_prefix}${command} <command name>` }, );

//Command usage
const usage = new Discord.MessageEmbed()
    .setColor('#fcfa65')
    .setTitle(`${Config.command_prefix}${command}`)
    .addFields({ name: "Usage", value: usageText });
    ...
    async execute(message, args) {
        ...
        if (currentSession.noSession()) {
            ...
            const sessionExist = new Discord.MessageEmbed()
                ...
                .addFields({ name: 'Starting Time', value: `${sessionInfo.startDateTime.format('DD/MMM/YYYY')}` }, { name: 'Ending Time', value: `${sessionInfo.endDateTime.format('DD/MMM/YYYY')}` }, { name: 'Session Duration', value: `${sessionInfo.duration.hours()} Hours ${sessionInfo.duration.minutes()} Minutes ${sessionInfo.duration.seconds()} Seconds` });
